...
export default class HellCore {
  ...
  constructor(logger: HellLogger) {
    ...
    this.client.on(Events.InteractionCreate, (interaction: Interaction) => {
      ...
      const command = this.chatInputCommands
        .get(interaction.commandName)
        .or(this.chatInputGuildCommands.get(interaction.commandName));
      if (command.isNone()) {
  ...
  async loadFeatures(): Promise<void> {
    const path = `${process.cwd()}/features`;
    for (const name of await readdir(path)) {
      const feature = (await import(`${path}/${name}/index.ts`)).default;
      try {
        feature.setup({
          client: this.client,
          logger: this.logger,
